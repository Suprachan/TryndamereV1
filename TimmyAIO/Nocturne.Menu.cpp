...
void __cdecl ::Tryndamere::Menu::MenuDraw(void* UserData)
{
	...
	if (ComboSpellTreeExpanded)
	{
		...
		SdkUiCheckbox("Use R in Combo", &UseROnLowHealth, &changed);
		if (changed)
		{
			MenuSaveSettings();
		}
		::Menu::SliderFloat("Use Q HealthPercent", "Menu.HealthQ", 25.0f, 50.0f, 100.0f);

		::Menu::SliderFloat("Use R HealthPercent", "Menu.HealthR", 25.0f, 50.0f, 100.0f);
...
if (MiscExpanded)
{
	...
	if (MiscExpandedHitchance)
	{
		...
		SdkUiCombo(" ", &TryndamereEPredHitchance, hitchanceList, RTL_NUMBER_OF(hitchanceList), &changed);
		if (changed)
		{
			MenuSaveSettings();
		}
